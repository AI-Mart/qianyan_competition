...
parser.add_argument("--init_from_opt", type=str, default=None, help="The path of checkpoint to be loaded.")
parser.add_argument("--seed", type=int, default=1000, help="random seed for initialization")
parser.add_argument('--device', choices=['cpu', 'gpu'], default="gpu", help="Select which device to train model, defaults to gpu.")
parser.add_argument("--input_file_train", type=str, required=True, help="The full path of input file")
parser.add_argument("--input_file_dev", type=str, required=True, help="The full path of input file")
...
def evaluate(model, criterion, metric, data_loader, phase="dev"):
    ...
    print("eval {} loss: {:.5}, accu: {:.5}".format(phase,
                                                    np.mean(losses), accu))
    with open('./eval_out.txt', 'a') as f:
        f.write("eval {} loss: {:.5}, accu: {:.5}".format(phase,
                                                    np.mean(losses), accu))
    model.train()
    metric.reset()
...
def do_train():
    ...
    train_ds, dev_ds = load_dataset("lcqmc", splits=["train", "dev"], data_files=[args.input_file_train, args.input_file_dev])
    ...
    if args.init_from_ckpt and os.path.isfile(args.init_from_ckpt):
        ...
        opt_state_dict = paddle.load(args.init_from_opt)
        optimizer.set_state_dict(opt_state_dict)
        path_list = args.init_from_ckpt.split('/')
        global_step = int(path_list[2].split('_')[1])
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        save_opt_path = os.path.join(save_dir, 'optimizer.pdopt')
